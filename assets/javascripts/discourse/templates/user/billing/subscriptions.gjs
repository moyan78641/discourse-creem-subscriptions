...
class SubscriptionPage extends Component {
  ...
  get statusLabel() {
    const s = this.subscription?.status;
    if (!s || s === "none") {
      return i18n("sparkloc.billing.status_none");
    }
    ...
    <div class="sparkloc-billing-page">
      ...
      {{#if this.isNone}}
        <div class="billing-card">
          ...
          <DButton @action={{this.subscribe}} @label="sparkloc.billing.subscribe" @icon="credit-card" @disabled={{this.subscribing}} class="btn-primary" />
        </div>
      ...
      {{else}}
        <div class="billing-card">
          ...
          <div class="billing-actions">
            <DButton @action={{this.openBillingPortal}} @label="sparkloc.billing.manage" @icon="credit-card" class="btn-default" />
            {{#if this.isActive}}
              <DButton @action={{this.requestCancel}} @label="sparkloc.billing.cancel" @disabled={{this.canceling}} class="btn-danger" />
            {{/if}}
          </div>
        </div>
      {{/if}}
