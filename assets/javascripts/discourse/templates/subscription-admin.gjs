...
class SubscriptionAdminPage extends Component {
  ...
  async cancelSubscription(username) {
    if (!confirm(`确定取消 ${username} 的订阅？将立刻移出订阅组。`)) {
      return;
    }
    ...
    <div class="subscription-admin-page">
      ...
      <div class="subscription-admin-add" style="margin-bottom:20px;padding:15px;background:var(--primary-very-low);border-radius:8px;">
        ...
        <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
          <input type="text" placeholder="用户名" value={{this.newUsername}} {{on "input" this.updateNewUsername}} style="padding:6px 10px;border:1px solid var(--primary-low);border-radius:4px;width:180px;" />
          <label>月数:</label>
          <input type="number" min="1" value={{this.newMonths}} {{on "input" this.updateNewMonths}} style="padding:6px 10px;border:1px solid var(--primary-low);border-radius:4px;width:70px;" />
          <button class="btn btn-primary" type="button" disabled={{this.loading}} {{on "click" this.addSubscription}}>添加</button>
        </div>
      </div>

      <div class="subscription-admin-renew-config" style="margin-bottom:15px;display:flex;gap:10px;align-items:center;">
        <label>续费月数:</label>
        <input type="number" min="1" value={{this.renewMonths}} {{on "input" this.updateRenewMonths}} style="padding:6px 10px;border:1px solid var(--primary-low);border-radius:4px;width:70px;" />
      </div>
          ...
          {{#each this.subscriptions as |sub|}}
            <tr style="border-bottom:1px solid var(--primary-low);">
              ...
              <td style="padding:8px;display:flex;gap:6px;">
                <button class="btn btn-small btn-primary" type="button" disabled={{this.loading}} {{on "click" (fn this.renewSubscription sub.username)}}>续费</button>
                <button class="btn btn-small btn-danger" type="button" disabled={{this.loading}} {{on "click" (fn this.cancelSubscription sub.username)}}>取消</button>
              </td>
            </tr>
          {{else}}
            <tr>
