...
module ::DiscourseSparklocBilling
  class CreemWebhookController < ::ApplicationController
    ...
    def handle_checkout_completed(object)
      ...
      SubscriptionStore.save(user.username, extract_subscription_data(object, "active"))
    ...
    def handle_subscription_paid(object)
      ...
      SubscriptionStore.save(user.username, extract_subscription_data(object, "active"))
      add_user_to_group(user)
      Rails.logger.info("[Creem] subscription.paid: #{user.username}")
    end
    ...
    def handle_subscription_trialing(object)
      ...
      SubscriptionStore.save(user.username, extract_subscription_data(object, "trialing"))
      add_user_to_group(user)
    ...
    def handle_subscription_canceled(object)
      ...
      attrs = extract_subscription_data(object, "canceled")
      attrs["canceled_at"] = object["canceled_at"] || Time.current.iso8601
      SubscriptionStore.save(user.username, attrs)
      Rails.logger.info("[Creem] subscription.canceled: #{user.username}")
    end
    ...
    def handle_subscription_scheduled_cancel(object)
      ...
      attrs["canceled_at"] = object["canceled_at"] || Time.current.iso8601
      SubscriptionStore.save(user.username, attrs)
      Rails.logger.info("[Creem] subscription.scheduled_cancel: #{user.username}")
    end
    ...
    def handle_subscription_expired(object)
      ...
      SubscriptionStore.save(user.username, extract_subscription_data(object, "expired"))
    ...
    def handle_subscription_paused(object)
      ...
      SubscriptionStore.save(user.username, extract_subscription_data(object, "paused"))
      remove_user_from_group(user)
    ...
    def handle_refund_created(object)
      ...
      SubscriptionStore.save(user.username, { "status" => "refunded" })
      remove_user_from_group(user)
